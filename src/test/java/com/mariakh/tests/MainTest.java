...
public class MainTest extends BaseTests {
    ...
    ProductCardPage productCardPage1 = new ProductCardPage();
    ProductCardPage productCardPage2 = new ProductCardPage();
    ...
    public void test() {
        ...
        startPage.getSearchBlock().searchProduct("iphone");
        searchResultPage.checkOpenSearchResultPage();
        searchResultPage.findProductByCodeAndClick("5072935");

        productCardPage1.checkOpenPage();
        productCardPage1.saveProductInfo();
        productCardPage1.clickGuaranteeButton("2");
        productCardPage1.selectNotFreeGuarantee();
        productCardPage1.clickBuy();

        productCardPage1.getSearchBlock().searchProduct("Apple AirPods Pro 2");

        productCardPage2.checkOpenPage();
        productCardPage2.saveProductInfo();
        productCardPage2.clickBuy();
        productCardPage2.getCartBlock().checkTotalAmount();
        productCardPage2.getCartBlock().clickOnCart();

        cartPage.checkProductPrices();
        cartPage.checkGuaranteePrices();
        cartPage.checkTotalAmount();
        cartPage.deleteProductFromCart("5072988");
        cartPage.checkTotalAmount();
...
/*        WebElement cartButton = driver.findElement(By.xpath("//a[@data-commerce-target='CART']"));
 ...
 /*       WebElement delAirPodsButton = cartItemProducts.get(1).findElement(By.xpath(".//button[@class='menu-control-button remove-button']"));
        ...
        //НИЖЕ Я НЕ ДЕЛАЛА НЕ ПОТЕРЯЙ АЛО
